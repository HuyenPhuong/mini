...
bool PhiTieuLayer::init() {
	...
	_background = Sprite::create("PhiTieu/cong_background.jpg");
	...
	//---------- ninja  -----------
	ninja = Ninja::create("PhiTieu/Ninja.png");
...
void PhiTieuLayer::PhongTieu(Vec2 dest) {
	...
	if (ninja->isAlive)
	{
		...
		ninja->fire(src, dest);
		CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("PhiTieu/Shuriken.mp3");
	}
}
...
void PhiTieuLayer::gameOver() {
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("PhiTieu/GameOverSound.mp3");
}
